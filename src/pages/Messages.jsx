...
const Messages = () => {
  ...
  return (
    <Layout title="Tin nhắn" description="Trò chuyện với các bạn sinh viên">
      <div className="h-[calc(100vh-120px)] bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex">
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            <div className="flex space-x-2 mb-4">
              <button
                ...
                className={`px-3 py-1 rounded-lg text-sm font-medium transition-colors ${filter === 'all' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
              >
                Tất cả
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg text-sm font-medium transition-colors ${filter === 'exchange' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
              >
                Đổi lớp
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-lg text-sm font-medium transition-colors ${filter === 'group' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
              >
                Ghép nhóm
              </button>
          ...
          <div className="flex-1 overflow-y-auto">
            {filteredContacts.map((contact) => (
              <div
                ...
                className={`p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors relative ${activeChat === contact.id ? 'bg-primary/5 border-r-2 border-r-primary' : ''
                  }`}
              >
                <div className="flex items-start space-x-3">
                  <div className="relative flex-shrink-0">
                    ...
                    {/* Unread badge - đã di chuyển vào đây và điều chỉnh position */}
                    {contact.unread > 0 && (
                      <div className="absolute -top-1 -right-1 w-5 h-5 bg-error text-white text-xs rounded-full flex items-center justify-center font-bold z-10">
                        {contact.unread}
                      </div>
                    )}
                  </div>
                    ...
                    {contact.type === 'exchange' && (
                      <div className="text-xs space-y-1">
                        <div className="flex items-center space-x-1">
                          <span className="text-success font-medium">Họ có:</span>
                          <span className="text-gray-500">{contact.classOffered}</span>
                        </div>
                        <div className="flex items-center space-x-1">
                          <span className="text-primary font-medium">Họ muốn:</span>
          ...
          {activeContact ? (
            <>
              ...
              <div className="p-6 border-b border-gray-200 bg-gray-50">
                    ...
                    <div className="relative">
                      <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center overflow-hidden">
                        {activeContact.avatar ? (
                          ...
                          <span className="text-white font-bold">{activeContact.name.charAt(0)}</span>
                        )}
                      </div>
                ...
                {activeContact.type === 'exchange' && (
                  <div className="mt-4 p-3 bg-white rounded-lg border border-gray-200">
                    <div className="flex items-center justify-between text-sm">
                      <div className="flex items-center space-x-2">
                        <span className="text-success font-medium">Họ có:</span>
                        <span className="text-gray-700 font-medium">{activeContact.classOffered}</span>
                      </div>
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-primary font-medium">Họ muốn:</span>
                        <span className="text-gray-700 font-medium">{activeContact.classWanted}</span>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {messages.map((message) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <p className={`text-xs mt-1 ${message.sender === 'me' ? 'text-primary-light' : 'text-gray-500'
                        }`}>
                        {message.time}
                      </p>
                    </div>
              ...
              {messages.some(m => m.type === 'invite') && (
                <div className="px-6 py-4 border-t bg-warning/10 border-warning/20">
                  <p className="text-warning font-medium mb-2">Bạn nhận được lời mời tham gia nhóm!</p>
                  <button
                    onClick={handleAcceptInvite}
                    className="bg-warning text-white px-4 py-2 rounded-lg hover:bg-warning-hover font-medium transition-colors"
                  >
                    Đồng ý tham gia
                  </button>
                  <button 
                    onClick={handleViewTeam}
                    className="bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary-hover font-medium transition-colors ml-2"
                  >
                    Xem chi tiết nhóm
                  </button>
                </div>
                ...
                <div className="flex items-end space-x-4">
                  ...
                  <div className="flex-1 relative">
                    <textarea
                      ...
                      className="w-full p-3 pr-12 border border-gray-200 rounded-xl resize-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
                      rows="1"
                    />
                  ...
                  <button
                    ...
                    className="bg-primary hover:bg-primary-hover text-white p-3 rounded-xl transition-colors"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <h3 className="text-lg font-bold text-gray-900 mb-2">Chọn một cuộc trò chuyện</h3>
